...
class Chessboard:
    ...
    def check_win(self, x, y):
        ...
        if (n_count + s_count + 1 >= 5) or (e_count + w_count + 1 >= 5) or (
                en_count + ws_count + 1 >= 5) or (wn_count + es_count + 1 >= 5):
    ...
    def handle_event(self, e):
        ...
        # 退出按钮
        if 690 <= mouse_pos[0] <= 790 and 625 <= mouse_pos[1] <= 675:
            click_music.play()
            sys.exit(1)

        # 重开按钮
        if 690 <= mouse_pos[0] <= 790 and 525 <= mouse_pos[1] <= 575 and self.piece_count > 0:
            self.grid = []
            for i in range(self.grid_count):
                self.grid.append(list('.' * self.grid_count))
            self.gameover = False
        ...
        # 悔棋按钮
        if 690 <= mouse_pos[0] <= 790 and 225 <= mouse_pos[
        ...
        # 认输按钮
        if 690 <= mouse_pos[0] <= 790 and 325 <= mouse_pos[1] <= 375 and not self.gameover:
            if self.piece == 'black':
        ...
        # 和棋
        if 690 <= mouse_pos[0] <= 790 and 425 <= mouse_pos[1] <= 475 and not self.gameover:
            self.black_win_count += 1
            self.white_win_count += 1
            self.grid = []
            for i in range(self.grid_count):
            ...
            self.gameover = False
            # ???
            self.piece = 'white' if self.piece=='white' else 'black'
            self.game_flag = 1
            click_music.play()
    ...
    def draw(self, screen):
        ...
        pg.draw.ellipse(screen, self.button_color, [690, 225, 100, 50])
        # pg.draw.rect(screen, self.button_color, [600, 350, 120, 80], 5)
        pg.draw.ellipse(screen, self.button_color, [690, 325, 100, 50])
        pg.draw.ellipse(screen, self.button_color, [690, 425, 100, 50])
        pg.draw.ellipse(screen, self.button_color, [690, 525, 100, 50])
        # pg.draw.rect(screen, self.button_color, [600, 450, 120, 80], 5)
        pg.draw.ellipse(screen, self.button_color, [690, 625, 100, 50])
        ...
        for r in range(self.grid_count):
            for c in range(self.grid_count):
                ...
                if piece != ".":
                    ...
                    # 画圆
                    pg.draw.circle(screen, color, [x, y], 12)
...
class Gomoku:
    def __init__(self):
        ...
        self.screen = pg.display.set_mode((850, 749))  # 初始化显示窗口
    ...
    def draw(self):
        ...
        # 插图
        img = pg.image.load('backgroud_gaitubao_850x749.jpg').convert_alpha()
        ...
        self.screen.blit(self.font2.render(f"{m} : {s} ", True, (0, 0, 0)), (710, 150))
        ...
        # 按钮文字
        self.screen.blit(self.font3.render("悔棋", True, (3, 22, 52)), (700, 225))
        self.screen.blit(self.font3.render("认输", True, (3, 22, 52)), (700, 325))
        self.screen.blit(self.font3.render("和棋", True, (3, 22, 52)), (700, 425))
        self.screen.blit(self.font3.render("重开", True, (3, 22, 52)), (700, 525))
        self.screen.blit(self.font3.render("退出", True, (3, 22, 52)), (700, 625))

        if self.chessboard.gameover:
            self.screen.blit(
                self.font1.render("{}获胜!".format("黑棋" if self.chessboard.winner == 'black'
                                                 else '白棋'), True, (8, 46, 84)), (660, 90))
