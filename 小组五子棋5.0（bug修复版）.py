...
piece_music = pg.mixer.Sound('2.wav')
...
class Chessboard:
    ...
    def handle_event(self, e):
        ...
        # 重开按钮
        if 600 <= mouse_pos[0] <= 720 and 350 <= mouse_pos[1] <= 430 and self.piece_count > 0:
            self.grid = []
            for i in range(self.grid_count):
                self.grid.append(list('.' * self.grid_count))
            self.gameover = False
        ...
        # 悔棋按钮
        if 600 <= mouse_pos[0] <= 720 and 250 <= mouse_pos[1] <= 430 and self.piece_count > 0:
...
class Gomoku:
    ...
    def draw(self):
        ...
        self.screen.blit(self.font1.render(f':{self.black_win_count}', True, (0, 0, 0)), (190, 15))
        pg.draw.circle(self.screen, (255, 255, 255), [290, 40], 15)
        self.screen.blit(self.font1.render(f':{self.white_win_count}', True, (0, 0, 0)), (310, 15))
